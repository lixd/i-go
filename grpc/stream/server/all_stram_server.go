...
*/
func (server *allStream) AllStream(stream proto.AllStreamServer_AllStreamServer) error {
	waitGroup := sync.WaitGroup{}

	waitGroup.Add(1)
	go func() {
		for i := 0; i < 5; i++ {
			err := stream.Send(&proto.AllStreamResp{Data: strconv.Itoa(i)})
			if err != nil {
...
func main() {
	...
	// 注册server
	proto.RegisterAllStreamServerServer(newServer, &allStream{})

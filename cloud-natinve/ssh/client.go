...
type SSH struct {
	...
	ExecTimeout       time.Duration `json:"execTimeout,omitempty" yaml:"execTimeout,omitempty"`
}
...
func (ss *SSH) connect(host string) (*ssh.Client, error) {
	...
	if ss.ConnectionTimeout == time.Duration(0) {
		ss.ConnectionTimeout = DefaultTimeout
	}
	if ss.ExecTimeout == time.Duration(0) {
		ss.ExecTimeout = DefaultTimeout
	}
	...
	client, err := SSHDialTimeout("tcp", addr, clientConfig, ss.ExecTimeout)
	if err != nil {

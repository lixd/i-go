...
func (v1Controller *V1Controller) RequestBodyHandler(c *gin.Context) {
	...
	if c.ShouldBindBodyWith(&user, binding.JSON) == nil {
		c.String(http.StatusOK, "the body should be LoginForm  user=%v password= %v", user, user.Password)
...
func (v1Controller *V1Controller) ParamsBind(c *gin.Context) {
	var form LoginForm
	err := c.Bind(&form)
	log.Printf("ShouldBind error= %v  form.user=%v form.password=%v", err, form, form.Password)
